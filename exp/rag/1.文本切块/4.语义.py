...
documents = SimpleDirectoryReader(input_files=["./白富美.txt"]).load_data()

# 创建语义分块器
splitter = SemanticSplitterNodeParser(
    # 默认值为1。这个参数控制评估语义相似度时，将多少个句子组合在一起当设置为1时，每个句子会被单独考虑当设置大于1时，会将多个句子组合在一起进行评估例如，如果设置为2，就会将每2个句子作为一个组来评估语义相似度
    buffer_size=2,  # 缓冲区大小
    # 默认值为95
    # 这个参数控制何时在句子组之间创建分割点,它表示余弦不相似度的百分位数阈值,当句子组之间的不相似度超过这个阈值时，就会创建一个新的节点
    # 数值越小，生成的节点就越多（因为更容易达到分割阈值）
    # 数值越大，生成的节点就越少（因为需要更大的不相似度才会分割）
    breakpoint_percentile_threshold=80,  # 断点百分位阈值
